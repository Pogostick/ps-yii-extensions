...
class ModelCode extends CCodeModel
{
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			array('tablePrefix, databaseComponent, baseClass, tableName, modelClass, modelPath', 'filter', 'filter'=>'trim'),
			array('tableName, databaseComponent, modelPath, baseClass', 'required'),
			array('tablePrefix, databaseComponent, tableName, modelPath', 'match', 'pattern'=>'/^(\w+[\w\.]*|\*?|\w+\.\*)$/', 'message'=>'{attribute} should only contain word characters, dots, and an optional ending asterisk.'),
	...
	public function init()
	{
		if(Yii::app()->getComponent($this->databaseComponent)===null)
			throw new CHttpException(500,'An active database ("db") connection is required to run this generator.');
		$this->tablePrefix=Yii::app()->getComponent($this->databaseComponent)->tablePrefix;
		parent::init();
	}
	...
	public function prepare()
	{
		...
		{
			$tables=Yii::app()->getComponent($this->databaseComponent)->schema->getTables($schema);
			if($this->tablePrefix!='')
			{
	...
	{
		return Yii::app()->getComponent($this->databaseComponent)->getSchema()->getTable($tableName);
	}
	...
	{
		if($addBrackets && Yii::app()->getComponent($this->databaseComponent)->tablePrefix=='')
			return $tableName;
		$prefix=$this->tablePrefix!='' ? $this->tablePrefix : Yii::app()->getComponent($this->databaseComponent)->tablePrefix;
		if($prefix!='')
		{
			if($addBrackets && Yii::app()->getComponent($this->databaseComponent)->tablePrefix!='')
			{
				$prefix=Yii::app()->getComponent($this->databaseComponent)->tablePrefix;
				$lb='{{';
				$rb='}}';
			}
			else
	...
	{
		$relations=array();
		foreach(Yii::app()->getComponent($this->databaseComponent)->schema->getTables() as $table)
		{
	...
	protected function generateRelationName($tableName, $fkName, $multiple)
	{
		...
		$table=Yii::app()->getComponent($this->databaseComponent)->schema->getTable($tableName);
		$i=0;
