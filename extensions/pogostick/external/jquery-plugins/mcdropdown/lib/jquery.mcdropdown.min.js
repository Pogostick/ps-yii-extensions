 ...
 * Rev:  1.2.10
 */
(function($){$.fn.mcDropdown=function(list,options){var dd;this.each(function(){dd=$.data(this,"mcDropdown");if(dd)return false;new $.mcDropDownMenu(this,list,options);});return dd||this;};$.mcDropdown={version:"1.2.10",setDefaults:function(options){$.extend(defaults,options);}};var defaults={minRows:8,maxRows:25,targetColumnSize:2,openFx:"slideDown",openSpeed:250,closeFx:"slideUp",closeSpeed:250,hoverOverDelay:200,hoverOutDelay:0,showFx:"show",showSpeed:0,hideFx:"hide",hideSpeed:0,dropShadow:true,autoHeight:true,lineHeight:19,screenPadding:10,allowParentSelect:false,delim:":",showACOnEmptyFocus:false,valueAttr:"rel",click:null,select:null,init:null,wrapperId:null,anchorId:null};var isIE6=($.browser.version&&$.browser.version<=6);$.mcDropDownMenu=function(el,list,options){var $self,thismenu=this,$list,$divInput,settings,typedText="",matchesCache,oldCache,$keylist,$keylistiframe,bInput,bDisabled=false;$self=$(el);bInput=$self.is(":input");settings=$.extend({},defaults,options);if(settings.click==null)settings.click=function(e,dropdown,settings){dropdown.setValue(this.attr(settings.valueAttr));}
$(document).bind("click",function(e){var $target=$(e.target);var $ul=$target.parents().filter(function(){return this===$list[0]||(!!$keylist&&$keylist[0]===this);});if($ul.length){var bIsParent=$target.is(".mc_parent");if(bIsParent&&$keylist&&$ul[0]===$keylist[0]){clearTimeout(iBlurTimeout);updateValue($target.find("> ul > li:first"),false);e.stopPropagation();return false;}
else if(!settings.allowParentSelect&&bIsParent)return false;if($target.not(".mc_root"))hideBranch.apply($target.parent().parent()[0],[e]);if(settings.click!=null&&settings.click.apply($target,[e,thismenu,settings])==false){return false;}}
thismenu.closeMenu();});$list=(((typeof list=="object")&&!!list.jquery))?list:$(list);$list.appendTo("body").css({position:"absolute",top:-10000,left:-10000}).find("ul").andSelf().css("display","block").each(function(){var $el=$(this);$el.data("width",$el[0].clientWidth);}).css({top:0,left:0,display:"none"});$list.find("> li").addClass("mc_root");$("li > ul",$list).parent().addClass("mc_parent");$divInput=$('<div class="mcdropdown"><a '+(settings.anchorId!==null?' id="'+settings.anchorId+'" ':'')+'href="#" tabindex="-1"></a><input type="hidden" name="'+(el.name||el.id)+'" id="'+(el.id||el.name)+'" /></div>').appendTo($('<div '+(settings.wrapperId!==null?'id="'+settings.wrapperId+'" ':'')+'style="position: relative;"></div>')).parent();var $input=$self.replaceWith($divInput).attr({id:"_mcdd_temp_input",name:"_mcdd_temp_input"});var $hidden=$divInput.find(":input");$divInput=$divInput.find(".mcdropdown").prepend($input);var $divInputClone=$divInput.clone().css({position:"absolute",top:-9999999,left:-999999,visibility:"visible"}).show().appendTo("body");var di={width:$divInputClone.width()-$("a",$divInputClone).width(),height:$divInputClone.outerHeight()}
$divInputClone.remove();$.data($hidden[0],"mcDropdown",thismenu);$divInput.parent().height(di.height);if($.browser.safari){setTimeout(function(){$self.width($divInput.width()-$("a",$divInput).width());},100);}
$self.width(di.width).filter(":input").attr("autocomplete","off").bind("keypress",checkKeypress).bind("mousedown",function(e){$(this).triggerHandler("focus");e.stopPropagation();return false;}).bind("contextmenu",function(){return false;}).bind("focus",onFocus).bind("blur",onBlur);if($.browser.msie||$.browser.safari)$self.bind("keydown",function(e){if(",8,9,37,38,39,40,".indexOf(","+e.keyCode+",")>-1)return checkKeypress(e);});$("a",$divInput).bind("click",function(e){if(bDisabled)return false;thismenu.openMenu(e);return false;});this.setValue=function(value,skipCallback){$hidden.val(value);var name=displayString(value);if(settings.select!=null&&skipCallback!=true)settings.select.apply(thismenu,[value,name]);return $self[bInput?"val":"text"](name);};if(bInput)this.setValue($self.attr("defaultValue"),true);this.getValue=function(value){return[$hidden.val(),$self[bInput?"val":"text"]()];};this.openMenu=function(e){if($list.is(":visible")){return(!!e)?thismenu.closeMenu():false;}
function open(){columnizeList($list).hide();addBindings($list);anchorTo($divInput.parent(),$list,true);$list.find(".mc_hover").removeClass("mc_hover");$list[settings.openFx](settings.openSpeed,function(){scrollToView($list);});if(isIE6&&!!$.fn.bgIframe)$list.bgIframe();}
if(e)open();else setTimeout(open,1);};this.closeMenu=function(e){$list.find("ul:visible").parent().each(function(){hideBranch.apply(this);});removeBindings($list);$list[settings.closeFx](settings.closeSpeed);};this.focus=function(){$self.focus();};this.disable=function(status){bDisabled=!!status;$divInput[bDisabled?"addClass":"removeClass"]("mcdropdownDisabled");$input.attr("disabled",bDisabled?"disabled":"");};function getNodeText($el){return $el.contents()[0]?$.trim($el.contents()[0].nodeValue):"";};function getTreePath($li){if($li.length==0)return[];var name=[getNodeText($li)];$li.parents().each(function(){var $el=$(this);if(this===$list[0])return false;else if($el.is("li"))name.push(getNodeText($el));});return name.reverse();};function displayValue(value){return getTreePath(getListItem(value));};function displayString(value){return displayValue(value).join(settings.delim);};function parseTree($selector){var s=[],level=(arguments.length>1)?++arguments[1]:1;$("> li",$selector).each(function(){var $self=$(this);var $ul=$("> ul",this);s.push({name:getNodeText($self),element:this,children:($ul.length)?parseTree($ul,level):[]});});return s;};function addBindings(el){removeBindings(el);$("> li",el).bind("mouseover",hoverOver).bind("mouseout",hoverOut);};function removeBindings(el){$("> li",el).unbind("mouseover",hoverOver).unbind("mouseout",hoverOut);};function scrollToView($el){var p=position($el,true);var sd=getScreenDimensions();if(p.bottom>sd.y){$("html,body").animate({"scrollTop":"+="+((p.bottom-sd.y)+settings.screenPadding)+"px"})}};function hoverOver(e){var self=this;var timer=$.data(self,"timer");if(!isNaN(timer))clearTimeout(timer);$(this).addClass("mc_hover");$.data(self,"timer",setTimeout(function(){showBranch.apply(self);},settings.hoverOverDelay));};function hoverOut(e){var self=this;var timer=$.data(self,"timer");if(!isNaN(timer))clearTimeout(timer);$(this).removeClass("mc_hover");$.data(self,"timer",setTimeout(function(){var $li=$(self);setTimeout(function(){if($li.parent().find("> li.mc_hover").length==0){$li.parents("li").each(function(){var self=this;clearTimeout($.data(self,"timer"));hideBranch.apply(self);if($(this).siblings().filter(".mc_hover").length>0)return false;});}},settings.hoverOverDelay);hideBranch.apply(self);},settings.hoverOutDelay));if(isIE6)e.stopPropagation();};function getShadow(depth){var shadows=$self.data("shadows");if(!shadows)
shadows={};if(!shadows[depth]){shadows[depth]=$('<div class="mcdropdown_shadow"></div>').appendTo('body');if(!!$.fn.bgIframe)shadows[depth].bgIframe();$self.data("shadows",shadows);}
return shadows[depth];};function showBranch(){var self=this;var $ul=$("> ul",this);if($ul.is(":visible")||($ul.length==0))return false;$(this).parent().find('> li ul:visible').not($ul).parent().each(function(){hideBranch.apply(this);});columnizeList($ul);addBindings($ul);var depth=$ul.parents("ul").length;var sd=getScreenDimensions();var li_coords=position($(this));$ul.css({top:li_coords.bottom,left:li_coords.marginLeft}).show();var menuBottom=$ul.outerHeight()+$ul.offset().top;if(menuBottom>sd.y){$ul.css("top",li_coords.bottom-(menuBottom-sd.y)-settings.screenPadding);}
var showShadow=function(){if(settings.dropShadow){var $shadow=getShadow(depth);var pos=position($ul);$shadow.css({top:pos.top+pos.marginTop,left:pos.left+pos.marginLeft,width:pos.width,height:pos.height}).insertAfter($ul).show();$.data(self,"shadow",$shadow);}}
if(settings.showSpeed<=0){showShadow();}else{$ul.hide()[settings.showFx](settings.showSpeed,showShadow);}};function hideBranch(){var $ul=$("> ul",this);if($ul.is(":hidden")||($ul.length==0))return false;if(settings.dropShadow&&$.data(this,"shadow"))$.data(this,"shadow").hide();if(isIE6)
$ul.css("visibility","hidden").parent().removeClass("mc_hover");$ul.stop()[settings.hideFx](settings.hideSpeed);};function position($el,bUseOffset){var bHidden=false;if($el.is(":hidden")){bHidden=!!$el.css("visibility","hidden").show();}
var pos=$.extend($el[bUseOffset===true?"offset":"position"](),{width:$el.outerWidth(),height:$el.outerHeight(),marginLeft:parseInt($.curCSS($el[0],"marginLeft",true),10)||0,marginRight:parseInt($.curCSS($el[0],"marginRight",true),10)||0,marginTop:parseInt($.curCSS($el[0],"marginTop",true),10)||0,marginBottom:parseInt($.curCSS($el[0],"marginBottom",true),10)||0});if(pos.marginTop<0)pos.top+=pos.marginTop;if(pos.marginLeft<0)pos.left+=pos.marginLeft;pos["bottom"]=pos.top+pos.height;pos["right"]=pos.left+pos.width;if(bHidden)$el.hide().css("visibility","visible");return pos;};function anchorTo($anchor,$target,bUseOffset){var pos=position($anchor,bUseOffset);$target.css({position:"absolute",top:pos.bottom,left:pos.left});return pos.bottom;};function getScreenDimensions(){var d={scrollLeft:$(window).scrollLeft(),scrollTop:$(window).scrollTop(),width:$(window).width(),height:$(window).height()};d.x=d.scrollLeft+d.width;d.y=d.scrollTop+d.height;return d;};function getPadding(el,name){var torl=name=='height'?'Top':'Left',borr=name=='height'?'Bottom':'Right';return(parseInt("0"+$.curCSS(el,"border"+torl+"Width",true),10)
