 ...
 */
	...
	var defaults = {
		...
		, wrapperId: null              // The id of the wrapper for more css control ...  added 2010-01-18 Jerry Ablan <jablan@pogostick.com>
		, anchorId: null			   // The id of the anchor for more css control  ...  added 2010-01-18 Jerry Ablan <jablan@pogostick.com>
	};
	...
	$.mcDropDownMenu = function(el, list, options){
		...
		$divInput = $('<div class="mcdropdown"><a ' + ( settings.anchorId !== null ? ' id="' + settings.anchorId + '" ' : '' ) + 'href="#" tabindex="-1"></a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name) + '" /></div>')
			.appendTo($('<div ' + ( settings.wrapperId !== null ? 'id="' + settings.wrapperId + '" ' : '' ) + 'style="position: relative;"></div>'))
			.parent();
			
		// get a reference to the input element and remove it from the DOM				
		var $input = $self.replaceWith($divInput).attr({id: "_mcdd_temp_input", name: "_mcdd_temp_input"});
