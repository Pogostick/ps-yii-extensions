...
class CPSPaypal extends CPSPaymentGateway
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->apiToken = CPSHelp::getOption( $_REQUEST, 'TOKEN', Yii::app()->user->getState( self::TOKEN ) );
		$this->apiPayerId = CPSHelp::getOption( $_REQUEST, 'PAYERID', Yii::app()->user->getState( self::PAYER_ID ) );
	...
	*/
	protected function setExpressCheckout( array $arRequestData = null )
	{
		...
		try
		{
			...
			{
				//	Store the token...
				$this->apiToken = CPSHelp::getOption( $_oResponse->cookedResponse, 'TOKEN', null );
				Yii::app()->user->setState( self::TOKEN, $this->apiToken );
				
				//	Redirect...
				if ( 'commit' == CPSHelp::getOption( $this->requestData, 'userAction', 'continue' ) )
	...
	*/
	public function getExpressCheckoutDetails()
	{
		...
		try
		{
			...
			{
				//	Store the payer ID
				$this->apiPayerId = CPSHelp::getOption( $_oResponse->cookedResponse, 'PAYERID', $this->apiPayerId );
				Yii::app()->user->setState( self::PAYER_ID, $this->apiPayerId );
