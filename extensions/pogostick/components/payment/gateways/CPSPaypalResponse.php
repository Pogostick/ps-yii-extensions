...
class CPSPaypalResponse extends CPSPaymentGatewayResponse
{
	...
	*/
	protected function parseErrors( array $arResponse, $bWarningsOnly = false )
	{
		...
		while ( true )
		{
			...
			$_sSeverity = strtolower( CPSHelp::getOption( $arResponse, "L_SEVERITYCODE{$_i}" ) );
			...
			$_oError->setErrorCode( CPSHelp::getOption( $arResponse, "L_ERRORCODE{$_i}" ) );
			$_oError->setSeverityCode( $_sSeverity );
			$_oError->setShortMessage( CPSHelp::getOption( $arResponse, "L_SHORTMESSAGE{$_i}" ) );
			$_oError->setLongMessage( CPSHelp::getOption( $arResponse, "L_LONGMESSAGE{$_i}" ) );
	...
	*/
	protected function parseResponse( array $arResponse )
	{
		...
		$_sAck = strtolower( CPSHelp::getOption( $arResponse, 'ACK' ) );
		$_sTimeStamp = CPSHelp::getOption( $arResponse, 'TIMESTAMP' );
		$_sCorrelationId = CPSHelp::getOption( $arResponse, 'CORRELATIONID' );		
		$_sVersion = CPSHelp::getOption( $arResponse, 'VERSION' );
		$_sBuild = CPSHelp::getOption( $arResponse, 'BUILD' );
